...
export default function Navigation() {
  ...
  const Suggestions = ({ suggestions }) => {
    ...
    return (
      <Box mt={2} bg="white" borderRadius="md" boxShadow="md" position="absolute" width="100%" p={'10px'}>
        <List>
          {suggestions.map((event) => (
            <ListItem key={event.id} className="name">{event.name}</ListItem>
          ))}
        </List>
      </Box>
    );
    ...
    <>
      <Box display={"flex"} justifyContent={"space-evenly"} bg={"#331F69"} alignItems={"center"} h={"10vh"}>
        ...
        <ButtonGroup>
          ...
          <Button bg={"#3E60C1"} color={"white"} className="btn-nav" onClick={openModalLogin}>
            LOGIN
          </Button>
          <Button bg={"#F7F7F7"} color={"#2e4583"} onClick={openModalRegister}>
            REGISTER
          </Button>
        </ButtonGroup>
      </Box>
      <Modal isOpen={isModalOpenLogin} onClose={closeModalLogin} isCentered>
        <ModalOverlay />
          ...
          <ModalBody>
            <Flex minH={"10vh"} align={"center"} justify={"center"} bg={useColorModeValue("whiteAlpha.50", "whiteAlpha.800")}>
              <Flex flexDirection="column" width="50wh" height="50vh" backgroundColor="transparent" justifyContent="center" alignItems="center">
                ...
                <Box>
                  New to us?{" "}
                  <Link color="blue.500" onClick={openModalRegister}>
                    Sign Up
                  </Link>
                </Box>
              </Flex>
      ...
      <Modal isOpen={isModalOpenRegister} onClose={closeModalRegister} isCentered>
        <ModalOverlay />
        <ModalContent bg="ghostwhite" size="xl">
          <ModalCloseButton />
          <ModalBody>
                ...
                <Box rounded={"lg"} bg="ghostwhite" p={8}>
                  <Stack spacing={4}>
                    ...
                    <Stack pt={6}>
                      <Text align={"center"}>
                        Already a user?{" "}
                        <Link color={"blue.400"} onClick={openModalLogin}>
                          Login
                        </Link>
                      </Text>
                    </Stack>
