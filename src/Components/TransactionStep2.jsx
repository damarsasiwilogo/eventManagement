...
import { setFormData, setDiscountedTotalPricesByReff, setDiscountedTotalPricesByCoupon, setDiscountedTotalPrices } from "../slices/transactionSlices"; // Import the Redux action
...
function TransactionStep2({ onNext, onPrevious }) {
  ...
  const validationSchema = Yup.object().shape({
    ...
    identitas: Yup.string()
      ...
      .matches(/^[0-9]+$/, "Identity number must contain only numbers"),
        ...
        <Flex mt={4} ml={10} flexDirection={"column"} mb={30}>
          ...
          >
            <Form>
                    ...
                    <Field name="date">
                      {({ field, form }) => (
                        <FormControl isInvalid={form.errors.date && form.touched.date}>
                          <Select {...field} placeholder="Date" value={formData.date} onChange={(e) => handleFormDataChange("date", e.target.value)}>
                    ...
                    <Field name="month">
                      {({ field, form }) => (
                        <FormControl isInvalid={form.errors.month && form.touched.month}>
                          <Select {...field} placeholder="Month" value={formData.month} onChange={(e) => handleFormDataChange("month", e.target.value)}>
                    ...
                    <Field name="year">
                      {({ field, form }) => (
                        <FormControl isInvalid={form.errors.year && form.touched.year}>
                          <Select {...field} placeholder="Year" value={formData.year} onChange={(e) => handleFormDataChange("year", e.target.value)}>
                            {years.map((year) => (
              ...
              <Box display={"flex"} bgColor="#EDEDED" justifyContent="flex-end" h={"10vh"} mt={20} mb={5}>
                <Button bg={"#F7F7F7"} color={"#2e4583"} size="sm" mr={4} mt={5} w={"90px"} onClick={onPrevious}>
                  Kembali
                </Button>

                <Button colorScheme="facebook" color={"white"} _hover={{ bg: "#24105c" }} size="sm" mr={20} mt={5} w={"90px"} type="submit">
                  Submit
                </Button>
              </Box>
            </Form>
